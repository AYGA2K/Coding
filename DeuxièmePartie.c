 ...
 Location *suprimer_location(Location*L, char CIN[50] )
{
   ...
   do
   {
    ...
    if (strcmp (tmp->client.CIN,client_chercher->client.CIN)==0 )
    {
       asup=tmp;
       asup->prev->next=asup->next;
       asup->next->prev=asup->prev;
       asup=NULL;
       free(asup);
       printf("Le client a ete supprime avec succes \n ");
...
{
 int i,j,n;
 struct List  *Location1;
 Location1=(Location*)malloc(sizeof(Location));
 Location1=NULL;
 struct List  *Location_chercher=(struct List *)malloc(sizeof(struct List ));
 int choix;
 char cin[50],name_file[50];
 char nom [50], prenom[50], CIN[50];
 Voiture *Voiture_loiee = (Voiture*)malloc(sizeof(Voiture)*100) ;
 float Prix_jour;
 int duree_location;
