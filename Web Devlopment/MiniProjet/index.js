...
function onSubmit(e) {
  ...
  if (returnTrip.checked === true)
    txt += "<p>This is a <strong>Return</strong> trip.</p>";
  if (onewayTrip.checked === true)
    txt += "<p>This is a <strong>One Way</strong> trip.</p>";
  txt +=
    "<p>Leaving on <strong>" +
    convertDate(leavingOn) +
    "</strong> from <strong>" +
    departFrom +
    "</strong>";
  if (returningOn.length > 1) {
    txt +=
      "<p>Returning on <strong>" +
      convertDate(returningOn) +
      "</strong> from <strong>" +
      arrivingAt +
      "</strong></p>";
  } else {
  ...
  txt +=
    "<p>Flying <strong>" +
    classType +
    "</strong> class with <strong>" +
    adults +
    "</strong> adults and <strong>" +
    children +
    "</strong> children.</p>";
...
function searchCities(e) {
  ...
  let url =
    ...
    query +
    "&types=(cities)&key=AIzaSyA3sUMaNBtaRTxi8KLraNrYCVga4xK55WE";
  ...
  xhr.onreadystatechange = function () {
    ...
    if (xhr.readyState === DONE) {
      if (xhr.status === OK) resp = JSON.parse(xhr.responseText);
      var data = resp.predictions;
      for (var i = 0; i < data.length; i++) {
        if (fromTo == "from") {
          country +=
            '<p id="country" onclick="addFromCity(this)">' +
            data[i].description +
            "</p>";
        }
        if (fromTo == "to") {
          country +=
            '<p id="country" onclick="addToCity(this)">' +
            data[i].description +
            "</p>";
        }
      }
...
document.getElementById("Vols").addEventListener("click", function () {
  document.getElementById("add_left ").style.visibility = "visible";
});
